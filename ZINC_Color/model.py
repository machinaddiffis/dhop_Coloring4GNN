...
from mlp import MLP
from pe import PEARLPositionalEncoder, GetSampleAggregator
...
def construct_model(cfg, list_create_mlp, deg1=None, peptides=False, **kwargs):
    ...
    if cfg.base_model == 'gine':
        ...
        print(cfg.n_base_layers, cfg.n_edge_types, cfg.node_emb_dims, cfg.base_hidden_dims, create_mlp,kwargs.get("residual"),kwargs.get('feature_type'),cfg.pooling)
        print("target_dim",target_dim, "bn",cfg.gine_model_bn, "pe_emb",cfg.pe_dims, "peptides",peptides)
...
class PEARL_GNN_Model(nn.Module):
    ...
    def forward(self, batch: Batch, W, final=True) -> torch.Tensor:
        ...
        X_n = self.node_features(batch.x.squeeze(dim=1))#embedding original node features
...
class GINEBaseModel(nn.Module):
    ...
    def __init__(
        ...
        """
        n_layers: GINE 网络的层数，指 GINE 模型中图卷积层的数量。
        n_edge_types: 边的类型数，用于处理不同类型的边特征。zinc的edge不同边数是3
        in_dims: 输入节点特征的维度。
        hidden_dims: 隐藏层的维度，表示 GINE 模型中每一层的隐藏状态大小。
        create_mlp: 一个可调用的函数，用于创建多层感知机（MLP）模型，通常是用来做最终的回归或分类任务。
        residual: 是否使用残差连接，通常残差连接可以帮助深层网络的训练。 如果我们是浅层的话，就得去掉
        bn: 是否使用批归一化（Batch Normalization），有助于训练稳定性。
        feature_type: 特征的类型，"discrete" 表示离散特征。
        pooling: 池化操作的类型，mean 表示使用均值池化，add 则使用加和池化。
        target_dim: 输出的维度，表示最终预测的输出类别数量（在图分类任务中通常为1）。
        pe_emb: 位置编码的维度（用于表示节点的位置编码）。37
        peptides: 是否使用肽链的编码（用于处理不同的分子结构）
        """
        super().__init__()
